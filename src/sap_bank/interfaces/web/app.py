...
Pensado para uso interno de contabilidad: subir Excel, previsualizar,
confirmar e insertar, ver resultados fila por fila y descargar el reporte.

Reutiliza exactamente la misma logica de negocio que la CLI:
  infrastructure.excel_reader  -> lee el Excel
  application.processor        -> parsea, construye payload, postea a SAP
  infrastructure.report_writer -> genera el .xlsx de conciliacion

Ejecucion (desde la raiz del proyecto):
    .venv\\Scripts\\python.exe -m src.sap_bank.interfaces.web.app
...
# ── Configuracion ──────────────────────────────────────────────────────────
# Ruta al config.yaml. Por defecto config/config.yaml en la raiz del proyecto.
CONFIG_PATH = os.environ.get("SAP_CONFIG", "config/config.yaml")

# Carpeta temporal para los Excel subidos mientras se procesan
...
@app.route("/")
def index():
    """Pagina principal: carga del archivo."""
...
def preview():
    """
    Recibe el Excel, lo lee y devuelve una previsualizacion de las filas
    SIN insertar nada en SAP. Permite al usuario revisar antes de confirmar.
    """
    ...
    # Guardar con un id unico para recuperarlo al confirmar
    ...
    # Construir previsualizacion: validar cada fila y armar el resumen
    cfg = load_config()
    ...
    # Guardar el id en la sesion para el confirm
    ...
    total_monto = sum(r["monto"] for r in preview_rows if r["valido"])
...
def process():
    """
    Confirma e inserta en SAP las filas del archivo previamente previsualizado.
    Devuelve el resultado fila por fila y la ruta del reporte generado.
    """
    ...
    # Cliente: real o dry-run segun el toggle de la interfaz
    if sap_real:
    ...
    # Guardar la ruta del reporte para descargarlo
...
def download_report():
    """Descarga el ultimo reporte .xlsx generado en esta sesion."""
...
if __name__ == "__main__":
    # host=0.0.0.0 permite acceso desde otras maquinas de la red interna.
    # Para uso solo local, cambiar a 127.0.0.1
